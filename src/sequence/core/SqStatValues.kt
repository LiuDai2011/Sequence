...
object SqStatValues {
    ...
    fun <T : UnlockableContent> ammo(map: ObjectMap<T, BulletType>, showUnit: Boolean = false): StatValue {
    ...
    fun ammoStat(`val`: Float): String =
        (if (`val` > 0) "[stat]+" else "[negstat]") + Strings.autoFixed(`val`, 1)

    fun multStat(`val`: Float): String =
        (if (`val` >= 1) "[stat]" else "[negstat]") + Strings.autoFixed(`val`, 2)

    fun icon(t: UnlockableContent): TextureRegion? = t.uiIcon

    object Bullets {
        fun trueDamage(bt: Table, amount: Float) =
            sep(bt, SqBundle.format("bullet.true-damage", amount))
    }
}
